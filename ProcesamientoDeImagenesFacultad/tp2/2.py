...
def procesar_patente(path, plot=False):
    ...
    AREA_MIN = 400
    filtro = (stats[:, cv2.CC_STAT_AREA] >= AREA_MIN) & (
...
def es_letra(stats, area_max=100, area_min=15):
    ...
    """
    return (
        stats[cv2.CC_STAT_AREA] < area_max and stats[cv2.CC_STAT_AREA] > area_min
    ) and (stats[cv2.CC_STAT_WIDTH] < stats[cv2.CC_STAT_HEIGHT])
...
def main():
    ...
    for recorte in recortes:
        plt.figure()
        plt.imshow(segmentar_caracteres(recorte, box=True), cmap="gray")
        plt.show()
