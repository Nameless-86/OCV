...
# --- Leo imagen --------------------------------------------------
img = cv2.imread(
    "xray-chest.png", cv2.IMREAD_GRAYSCALE
...
)  # Cualquier valor fuera del rango se satura.
plt.colorbar(h)
plt.title("Imagen")
plt.xlabel("X")
plt.ylabel("Y")
...
img90_RMSE = np.sqrt(
    np.sum(np.power(img.astype(float) - img90.astype(float), 2))
) / np.sqrt(np.size(img))
img05_RMSE = np.sqrt(
    np.sum(np.power(img.astype(float) - img05.astype(float), 2))
) / np.sqrt(np.size(img))
img05_RMSE_v2 = np.sqrt(
    np.mean((img.astype(float) - img05.astype(float)) ** 2)
)  # Otra forma...
...
img = cv2.normalize(
    img.astype("float"), None, 0.0, 1.0, cv2.NORM_MINMAX
)  # Así no, porque lleva los valores min/max a 0/1
img.min()
img.max()
h = plt.imshow(imgd, cmap="gray")
plt.colorbar(h)
